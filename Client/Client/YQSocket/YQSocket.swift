...
protocol YQSocketDelegate : class {
    func socket(_ socket : YQSocket, joinRoom user : UserInfo)
    func socket(_ socket : YQSocket, leaveRoom user : UserInfo)
    func socket(_ socket : YQSocket, chatMsg : ChatMessage)
    func socket(_ socket : YQSocket, giftMsg : GiftMessage)
}

class YQSocket {
    weak var delegate : YQSocketDelegate?
    fileprivate var tcpClient : TCPClient
    fileprivate var userInfo : UserInfo.Builder = {
        let userInfo = UserInfo.Builder()
        userInfo.name = "wYQ\(arc4random_uniform(10))"
        userInfo.level = 20
...
extension YQSocket {
    ...
    func sendTextMsg(message : String) {
        ...
        // 2.获取对应的data
        let chatData = try? chatMsg.build()
        
        // 3.发送消息到服务器
        sendMsg(data: (chatData?.data())!, type: 2)
    }
    ...
    func sendGiftMsg(giftName : String, giftURL : String, giftCount : Int) {
        ...
        guard let data = giftData else {
            return;
        }
        
        // 3.发送礼物消息
        sendMsg(data: data, type: 3)
    }
